...
build_server:
	go build -o userve github.com/murphybytes/ucp/server; ln -sf $(shell pwd)/userve $(GOPATH)/bin/.
	go build -o ucp_file_reader github.com/murphybytes/ucp/server/ucp_file_reader; ln -sf $(shell pwd)/ucp_file_reader $(GOPATH)/bin/.
	go build -o ucp_file_writer github.com/murphybytes/ucp/server/ucp_file_writer; ln -sf $(shell pwd)/ucp_file_writer $(GOPATH)/bin/.
